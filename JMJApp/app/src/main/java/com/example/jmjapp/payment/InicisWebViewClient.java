...
public class InicisWebViewClient extends WebViewClient {
    ...
    protected boolean handleNotFoundPaymentScheme(String scheme) {
        ...
        if (com.example.jmjapp.payment.PaymentScheme.ISP.equalsIgnoreCase(scheme)) {
            activity.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=" + com.example.jmjapp.payment.PaymentScheme.PACKAGE_ISP)));
            return true;
        } else if (com.example.jmjapp.payment.PaymentScheme.BANKPAY.equalsIgnoreCase(scheme)) {
            activity.startActivity(new Intent(Intent.ACTION_VIEW, Uri.parse("market://details?id=" + com.example.jmjapp.payment.PaymentScheme.PACKAGE_BANKPAY)));
            return true;
        }
