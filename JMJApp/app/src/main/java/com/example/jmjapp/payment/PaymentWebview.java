...
public class PaymentWebview extends Activity {
    ...
    private String shopNumber, jwt, resId, resDate, resTime, resShop, resAddr, resName, resPhone, qr;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        if (qr == null) {
            ...
            if (intentData == null) {
                ...
                //paymentWebView.loadUrl("http://192.168.1.51:8088/androidPayment");
            ...
            Log.d("resPrice", String.valueOf(resPrice));

            Uri intentData = intent.getData();

            if (intentData == null) {
                paymentWebView.addJavascriptInterface(new MyJavaScriptInterface(), "android");
                //paymentWebView.loadUrl("http://192.168.1.51:8088/androidQrPayment");
                paymentWebView.loadUrl("http://3.34.55.186:8088/androidQrPayment");
            } else {
                //isp 인증 후 복귀했을 때 결제 후속조치
                Log.d("실행2", "실행2");
                String url = intentData.toString();
