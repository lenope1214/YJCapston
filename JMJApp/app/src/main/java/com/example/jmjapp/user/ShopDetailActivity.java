...
//        shop_detail_reply = (TextView) findViewById(R.id.shop_detail_reply);
        ...
        shop_detail_phonecall.setOnClickListener(v -> Toast.makeText(ShopDetailActivity.this, "전화", Toast.LENGTH_SHORT).show());
        ...
        getMarks.enqueue(new Callback<Mark.MarkList>() {
            ...
            public void onResponse(Call<Mark.MarkList> call, Response<Mark.MarkList> response) {
                if (response.isSuccessful()) {
                    System.out.println("찜 목록 조회 성공");
                    List<Shop> markList = response.body().getShopList();

                    String list_shopNumber[] = new String[markList.size()];

                    int index = 0;

                    for (Shop list : markList) {
//                        list_shopNumber[index] = list.getShopId();
//                        index++;
                        if (list.getShopId().equals(shopNumber)) {
                            System.out.println("찜한 매장");
                            shop_detail_zzim_img.setImageResource(R.drawable.zzimred);
                            zzim = false;
                        } else {
                            System.out.println("찜하지 않은 매장");
                            shop_detail_zzim_img.setImageResource(R.drawable.zzimblack);
                            zzim = true;
                        }
                    }

//                    for (int i = 0; i < list_shopNumber.length; i++) {
//                        if (list_shopNumber[i].equals(shopNumber)) {
//                            shop_detail_zzim_img.setImageResource(R.drawable.zzimred);
//                            zzim = false;
//                        }
//                    }
...
//   찜 ♥
        shop_detail_zzim.setOnClickListener(v -> {
        ...
        order_basket.setOnClickListener(v -> {
        ...
        shopCall.enqueue(new Callback<Shop>() {
            ...
            public void onResponse(Call<Shop> call, Response<Shop> response) {
                if (response.code() == 200) {
                    ...
                    if (phone != null) {
                        shop_detail_phonecall.setOnClickListener(v -> {
